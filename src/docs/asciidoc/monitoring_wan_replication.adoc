...
In this page, you see **WAN Replication Operations Table** for
...
cluster, consistency check will be ignored. If it's enabled for the
source cluster but not for the target cluster,
all entries will be reported as if they need a sync because a sync
