...
enable the "Start TLS" field. See <<ldap-ssl, LDAP Authentication>>.
* If you're using Active Directory authentication, make sure you use Java's
truststore related system properties. See <<ad-ssl, Active Directory Authentication>>.
...
* `MC-0007 [Config]`: Socket interceptor is enabled.
* `MC-0008 [Config]`: Socket interceptor is disabled.
...
* `MC-1002 [Cluster Config]`: URL of Management Center is changed.
...
* `-Dhazelcast.mc.mail.connectiontimeout` sets the socket connection timeout for initiating the
SMTP session, in milliseconds. Its default value is 7000.
* `-Dhazelcast.mc.mail.timeout` sets the socket read timeout during the session, in milliseconds.
Its default value is 7000.
