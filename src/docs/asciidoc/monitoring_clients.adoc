...
(maximum, used and total memory). You can open both of these
charts as separate windows using the image:ChangeWindowIcon.jpg[change window]
button placed at top right of each chart; this gives you a
...
* **Type**: Type of the client. Java client is the only
supported client type at the moment.
...
Next to the **General** tab, the **Runtime** tab shows the
...
The **Client Near Cache Statistics** table shows statistics related
