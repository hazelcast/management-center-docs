...
To start Management Center with a valid Enterprise Edition license, you can pass the environment variable `MC_LICENSE`:

[source,shell,subs="attributes+"]
----
docker run --rm -p 8080:8080 -e MC_LICENSE=<your-licence-key> hazelcast/management-center:{full-version}
----
...
[[starting-with-enterprise-license]]
== Starting Management Center with an Enterprise license
If you want to start Management Center with a valid Enterprise Edition license via the command line,
you can set the `MC_LICENSE` or pass the Java property `hazelcast.mc.license` environment variable (see xref:deploy-manage:system-properties.adoc#starting-with-a-license[`hazelcast.mc.license` property] for more details).
...
[source,shell,subs="attributes+"]
----
SET MC_LICENSE=<license-key>
mc-start.cmd
----
--
====

Alternatively, you can set the license via the UI as documented in the xref:deploy-manage:license-management.adoc[] section.
