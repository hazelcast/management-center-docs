...
To start Management Center with a valid Enterprise License you can either pass the environment variable `MC_LICENSE`:
...
you could set the `MC_LICENSE` or pass the java property `hazelcast.mc.license` environment variable (see xref:system-properties.adoc#starting-with-a-license[`hazelcast.mc.license` property] for more details).
