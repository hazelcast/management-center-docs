...
[[map-index-stats]]
**Map Index Stats Table** provides information about Map Index statistics if there exists any index for this map.

NOTE: You need to create indexes to see this table here. See the xref:{page-latest-supported-hazelcast}@hazelcast:query:indexing-maps.adoc[Indexing Maps] for details.

image:ROOT:MapIndexStatsTable.png[Map Index Stats Table]

image:ROOT:MapIndexStatsTable2.png[Map Index Stats Table-2]

You can see more details about an index by clicking on the down arrow next to the index name.

From left to right this table lists the following:

.Map Index Stats Table Fields
[cols="20%s,80%a"]
|===
|Field|Description
...
|Total Memory Cost
|Total local memory cost of the index.

|Total Query Count
|Total number of queries served by the index.

|Total Insert Count
|Total number of insert operations performed on the index.

|Average Insert Time
|Average time of insert operations performed on the index among the members.

|Total Update Count
|Total number of update operations performed on the index.

|Average Update Time
|Average time of update operations performed on the index among the members.

|Total Remove Count
|Total number of remove operations performed on the index.

|Average Remove Time
|Average time of remove operations performed on the index among the members.

|Total Hit Count
|Total number of index hits.

|Average Hit Time
|Average time of index hits performed on the index among the members.

|Average Hit Selectivity
|Average selectivity of the hits served by the index among the members.
|===

.Map Index Stats Table Detailed Sub Row Fields
[cols="20%s,80%a"]
|===
|Field|Description
