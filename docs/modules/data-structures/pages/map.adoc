...
NOTE: Map config is updated only on the members MC is aware of at the time of the update. When a new member joins the cluster, the updated map configuration is not propagated to it.
Also if a member has persistence enabled, the updated map config is not persisted to disk and the updates are lost upon the next restart.
