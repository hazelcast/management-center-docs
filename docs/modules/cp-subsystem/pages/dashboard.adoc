...
If a Hazelcast instance that owns a CP session is not temporarily partitioned and crashed, its session could be closed manually to release its resources immediately.

This action is available only to admin users.

. Go to *CP Subsystem* > *Dashboard*.

. Find the CP session that you want to close from the CP Session Metrics Table.

. Click `Terminate` button in the Actions column for the related session.

If everything goes well, a success message is displayed.

image:ROOT:CPTerminateSessionDialog.png[Promote Member to CP Confirmation]
