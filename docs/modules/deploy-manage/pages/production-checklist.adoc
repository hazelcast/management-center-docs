...
=== Reverse proxies and load balancers (WebSocket)

Management Center uses WebSocket connections for some of the features. If a reverse proxy or load balancer does not support WebSockets or is not configured to preserve the WebSocket handshake headers, you may see errors. When using a reverse proxy or load balancer in front of Management Center, make sure to:

- Ensure your proxy/load balancer supports WebSocket connections.
- Make sure the WebSocket upgrade headers are forwarded intact by the proxy: `Upgrade: websocket` and `Connection: Upgrade`.
