...
xref:system-properties.adoc#hazelcast-mc-session-timeout-seconds[`hazelcast.mc.session.timeout.seconds` system property]. For example, the following command starts Management Center with
...
the `hazelcast.mc.allowMultipleLogin=false` system property.
...
You can configure this behavior, using the following system properties:
...
Center with the `hazelcast.mc.forceLogoutOnMultipleLogin=true` system property.