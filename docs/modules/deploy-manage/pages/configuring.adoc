...
|xref:declarative-configuration.adoc[YAML declarative configuration file] (Beta)
|Declare cluster connections, the security provider, local users, sensitive-properties masking, and Prometheus authentication in a single file that Management Center applies automatically every time it starts. A GitOps-friendly alternative to `hz-mc conf` for these entities.
...
- xref:declarative-configuration.adoc#clusters[YAML declarative configuration file] (Beta; either the cluster name and member addresses, or a path to a client configuration file)
