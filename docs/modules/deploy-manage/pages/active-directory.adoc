...
To set up Active Directory, you need to configure settings in the UI, the `hz-mc conf` tool, or a YAML declarative configuration file.
...
+
--
Declare an `activeDirectory` block under `securityProvider` in a xref:declarative-configuration.adoc#active-directory[YAML declarative configuration file]:
...
You can also use `active-directory configure --force` to reconfigure Active Directory without running `security reset` first.
...
== Using --force to configure Active Directory

Use the `-f` / `--force` flag with `active-directory configure` to make configuration idempotent:

* If Active Directory is not yet configured, it is set up.
* If Active Directory is already configured with the same settings, the command succeeds without making any changes.
* If Active Directory is already configured with different settings, the existing configuration is overwritten without needing to run `security reset` first.
