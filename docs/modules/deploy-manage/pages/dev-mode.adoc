...
+
--
To set up dev mode with a xref:declarative-configuration.adoc[YAML declarative configuration file], declare an empty `devMode` block under `securityProvider`:
...
For details about the `hz-mc conf` tool, see xref:mc-conf.adoc[]. For details about the YAML declarative configuration file, including how to switch from dev mode to another provider without a manual reset step, see xref:declarative-configuration.adoc[] and xref:declarative-configuration-apply-mode.adoc[].
