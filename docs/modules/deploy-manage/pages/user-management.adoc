...
[IMPORTANT]
====
The xref:tools:scripting.adoc[Scripting] and xref:tools:console.adoc[Console] features are an exception to the role-based permissions shown above. When these features are enabled on the cluster, any user who can access them has full, unrestricted access to the cluster, regardless of their assigned Management Center role. If granular access control is critical, consider leaving scripting and the console disabled.
====
