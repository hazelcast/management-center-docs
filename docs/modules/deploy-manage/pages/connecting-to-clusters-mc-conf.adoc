...
TIP: You can also preconfigure a cluster connection with a xref:declarative-configuration.adoc#clusters[YAML declarative configuration file] instead of `hz-mc conf`, which supports both options above -- the cluster name and member addresses, or a path to a client configuration file. This lets Management Center apply the connection automatically every time it starts, instead of requiring a one-off `hz-mc conf` run. See xref:declarative-configuration.adoc[].
...
. [[using-a-client-configuration-file]]To provide the path to a client configuration file, use the following:
+
```bash
...
For a fully declarative, GitOps-friendly alternative that also covers cluster connections, security providers, and more, see xref:declarative-configuration.adoc[YAML declarative configuration (Beta)].
