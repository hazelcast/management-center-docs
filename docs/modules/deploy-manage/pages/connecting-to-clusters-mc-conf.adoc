...
TIP: You can also preconfigure a cluster connection with a xref:declarative-configuration.adoc#clusters[YAML declarative configuration file] instead of `hz-mc conf`, which supports the cluster name and member addresses form only. This lets Management Center apply the connection automatically every time it starts, instead of requiring a one-off `hz-mc conf` run. See xref:declarative-configuration.adoc[].
