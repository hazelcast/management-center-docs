...
=== Use --force to add or overwrite a cluster connection
...
.Linux and Mac
```bash
hz-mc conf cluster add --force -cn={cluster name} -ma={member addresses}
```

.Windows
```bash
mc-conf.bat cluster add --force -cn={cluster name} -ma={member addresses}
```
...
=== Use --force to remove a cluster connection
...
.Linux and Mac
```bash
hz-mc conf cluster remove --force -cn={cluster name}
```

.Windows
```bash
mc-conf.bat cluster remove --force -cn={cluster name}
```
