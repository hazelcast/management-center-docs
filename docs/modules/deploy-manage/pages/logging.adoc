...
Management Center with the `hazelcast.mc.log.level` system property. For example, to use the debug logging level, start Management Center with the following line:
...
To enable audit logging, start Management Center with the xref:system-properties.adoc#hazelcast-mc-auditlog-enabled[`hazelcast.mc.auditlog.enabled` system property] set to `true`.
