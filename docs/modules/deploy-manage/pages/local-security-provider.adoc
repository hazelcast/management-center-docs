...
To set up the local security provider, you need to create an admin user, using the UI, the `hz-mc conf` tool, or a YAML declarative configuration file.
...
+
--
To set up the local security provider with a xref:declarative-configuration.adoc[YAML declarative configuration file], declare the users you want under `securityProvider.local.users`:
...
A `file:<path>` password value is resolved from that file's contents at load time -- see xref:declarative-configuration.adoc#secrets-the-file-convention[Secrets: the `file:` convention].
--
====
...
To create additional users, admin users can use the UI or you can use the `hz-mc conf` tool. You can also list additional users under `securityProvider.local.users` in a xref:declarative-configuration.adoc[YAML declarative configuration file (Beta)]; with `applyMode: upsert`, reapplying the file on a later restart creates any new users it lists.
...
== Using --force to configure the local security provider

Use the `-f` / `--force` flag with the `user configure` task to make configuration idempotent:

* If the local security provider is not yet configured, it is set up.
* If the local security provider is already configured with the same settings, the command succeeds without making any changes.
* If the local security provider is already configured with different settings, the existing configuration is overwritten without needing to run `security reset` first.
* If a *different* security provider is currently active, the command fails with an error even when `--force` is used.

== Next steps

For details about the `hz-mc conf` tool, see xref:mc-conf.adoc[]. For details about the YAML declarative configuration file, see xref:declarative-configuration.adoc[].
