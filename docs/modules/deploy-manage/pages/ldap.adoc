...
To set up LDAP, you need to configure settings in the UI, the `hz-mc conf` tool, or a YAML declarative configuration file.
...
+
--
Declare an `ldap` block under `securityProvider` in a xref:declarative-configuration.adoc#ldap[YAML declarative configuration file]:
...
An optional `keystore` block covers <<password-encryption,password encryption>> -- see xref:declarative-configuration.adoc#ldap[the full field reference].
--
====
...
You can also use `ldap configure --force` to reconfigure LDAP without running `security reset` first.
...
== Using --force to configure LDAP

Use the `-f` / `--force` flag with `ldap configure` to make configuration idempotent:

* If LDAP is not yet configured, it is set up.
* If LDAP is already configured with the same settings, the command succeeds without making any changes.
* If LDAP is already configured with different settings, the existing configuration is overwritten without needing to run `security reset` first.
