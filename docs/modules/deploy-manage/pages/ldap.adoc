...
the `uid` attribute.
+
NOTE: You can use the xref:system-properties.adoc#hazelcast-mc-ldap-timeout[`hazelcast.mc.ldap.timeout` system property] to
...
start it with the `hazelcast.mc.askKeyStorePassword` system property. Management Center
...
`hazelcast.mc.keyStore.path` system property.
