...
To set up SAML, you need to configure settings in the UI, the `hz-mc conf` tool, or a YAML declarative configuration file.
...
+
--
Declare a `saml` block under `securityProvider` in a xref:declarative-configuration.adoc#saml[YAML declarative configuration file]:
...
== Using --force to configure SAML

Use the `-f` / `--force` flag with `saml configure` to make configuration idempotent:

* If SAML is not yet configured, it is set up.
* If SAML is already configured with the same settings, the command succeeds without making any changes.
* If SAML is already configured with different settings, the existing configuration is overwritten without needing to run `security reset` first.
