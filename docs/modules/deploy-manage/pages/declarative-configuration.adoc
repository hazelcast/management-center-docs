...
|`clusters`
|xref:connecting-to-clusters-mc-conf.adoc[] (the member-addresses form only, see <<clusters>>)
...
The `clusters` key mirrors the member-addresses form of `hz-mc conf cluster add` -- see xref:connecting-to-clusters-mc-conf.adoc[]. The client-configuration-file form (XML or YAML client config, used for TLS or other advanced client settings) isn't supported here; use xref:connecting-to-clusters-ui.adoc#using-a-client-configuration-file[the UI] or xref:connecting-to-clusters-mc-conf.adoc#using-a-client-configuration-file[`hz-mc conf`] for that instead.
...
|`memberAddresses`
|List of one or more member addresses. Required.

|`enabled`
|Whether Management Center tries to connect on startup. Default: `true`.
|===

Two `clusters` entries sharing the same `name` in the same file are always rejected.
