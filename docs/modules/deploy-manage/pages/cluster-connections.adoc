...
** xref:declarative-configuration.adoc#clusters[YAML declarative configuration file (Beta)]
...
NOTE: The YAML declarative configuration file doesn't support uploading a client configuration file, so it can't be used to configure TLS for a cluster connection. Use the UI or `hz-mc conf` instead.
