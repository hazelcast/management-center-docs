...
You can also preconfigure any security provider with xref:mc-conf.adoc[`hz-mc conf`] or a xref:declarative-configuration.adoc#security-provider[YAML declarative configuration file (Beta)], so it's set up automatically before Management Center starts.
