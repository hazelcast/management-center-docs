...
To use the `mc-conf` tool, execute the `mc-conf` script in the `bin/` directory of the Management Center distribution. This script is available as a shell script (`mc-conf.sh`) for Linux and Mac or a batch file (`mc-conf.bat`) for Windows.
