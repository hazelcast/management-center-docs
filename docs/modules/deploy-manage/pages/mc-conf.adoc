...
TIP: For a fully declarative, GitOps-friendly alternative that reuses the same underlying configuration logic as this tool, see xref:declarative-configuration.adoc[YAML declarative configuration (Beta)].
...
TIP: This can also be set with the `sensitiveProperties` key of a xref:declarative-configuration.adoc#sensitive-properties[YAML declarative configuration file (Beta)].
...
Use the `-v` or `--verbose` flag to enable full logging output, including full stack traces.

[source,bash]
----
hz-mc conf cluster add --cluster-name=dev --cluster-address=localhost:5701 --verbose
----

== Lenient mode

Use the `-l` or `--lenient` flag to exit with code `0` when trying to add an object that already exists, instead of failing with an error. This is useful in automation scripts where you want the command to fail silently if the object is already configured.

[source,bash]
----
hz-mc conf cluster add --cluster-name=dev --cluster-address=localhost:5701 --lenient
----
