...
|[[hazelcast-mc-config-file]]hazelcast.mc.config.file

HAZELCAST_MC_CONFIG_FILE
|Path to a YAML declarative configuration file that Management Center applies automatically before it starts. Not set by default. See xref:declarative-configuration.adoc[YAML declarative configuration (Beta)].
|
[source,bash,subs="attributes+"]
----
...
|[[hazelcast-mc-info-instanceName]]hazelcast.mc.info.instanceName
|Set the instance name visible in the xref:getting-started:user-interface.adoc#top-bar[Top Bar]. Set blank to not display any instance name. If not set the instance name defaults to the hostname of the machine Manage Center is deployed on.
|
[source,bash,subs="attributes+"]
----
hz-mc start -Dhazelcast.mc.info.instanceName=ManCenter-us-east-1
----
