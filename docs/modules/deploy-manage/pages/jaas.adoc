...
To set up JAAS, you need to configure settings in the UI, the `hz-mc conf` tool, or a YAML declarative configuration file.
...
+
--
Declare a `jaas` block under `securityProvider` in a xref:declarative-configuration.adoc#jaas[YAML declarative configuration file]:
