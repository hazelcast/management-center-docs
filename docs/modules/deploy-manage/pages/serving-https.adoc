...
- Disable hostname verification with the following system property:
+
```
...
system property when starting the Management Center. For example:
...
have an open HTTP port that redirects to the HTTPS port, set the xref:system-properties.adoc#hazelcast-mc-tls-enablehttpport[`hazelcast.mc.tls.enableHttpPort` system property] to `true`.
...
you need to use the following system property:
