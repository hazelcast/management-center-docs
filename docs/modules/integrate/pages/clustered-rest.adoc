...
IMPORTANT: Encoding does not make every character usable: a name that contains a percent sign (`%`, encoded as `%25`) or a backslash (`\`, encoded as `%5C`) will be rejected with `400 Bad Request`. For guidance on choosing names, see xref:hazelcast:data-structures:distributed-data-structures.adoc#naming-data-structures[Naming Data Structures].
