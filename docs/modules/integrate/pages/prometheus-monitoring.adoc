...
TIP: You can also set this with the `prometheusAuth` key of a xref:deploy-manage:declarative-configuration.adoc#prometheus-auth[YAML declarative configuration file (Beta)], so Management Center applies it automatically on every start.
