...
successful is shown. See xref:troubleshoot:hazelcast-clusters.adoc#running-garbage-collection[Running Garbage Collection].
* **Thread Dump**: Takes a thread dump of the
selected member and shows it in a separate dialog. See xref:troubleshoot:hazelcast-clusters.adoc#taking-a-thread-dump[Taking a Thread Dump].
