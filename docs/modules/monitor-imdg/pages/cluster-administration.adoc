...
NOTE: CP subsystem management operations require enabled REST API in the IMDG cluster. See the {imdg-docs}#rest-client[IMDG documentation]
for more information.

The **CP Subsystem** tab can be used to monitor overall status of the
{imdg-docs}#cp-subsystem[CP subsystem]
...
{imdg-docs}#cp-subsystem-configuration[configured CP members count].
