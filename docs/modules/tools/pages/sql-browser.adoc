...
If you run *simple SELECT queries (without JOIN and AGGREGATION)* over IMap Mappings then you can get access to xref:data-structures:map.adoc#map-browser[query medata].
image:ROOT:SQLBrowserQueryResultTabWithClosedMapBrowser.png[A list of query results with chevron buttons]

By clicking on *chevron button* at the beginning of each SQL result table row, metadata view opens below a row
image:ROOT:SQLBrowserQueryResultTabWithOpenMapBrowser.png[A list of query results with metada view]
