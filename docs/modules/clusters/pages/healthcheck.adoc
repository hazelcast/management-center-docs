...
The sidebar navigation item provides notifications about the most severe reported issues, whether they are failures or warnings. These notifications are symbolized by icons: red for failures and yellow for warnings. Furthermore, a tooltip presents the total count of reported potential issues.
