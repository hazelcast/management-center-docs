...
== Excluding Checks
You can exclude a check from the Healthcheck analysis by adding the check ID to the `hazelcast.mc.clusterHealthCheck.exclude` system property.
An excluded check does not exist in Healthcheck report, and the check will not be performed on any member in the cluster.
See xref:deploy-manage:system-properties.adoc[] for more information.
